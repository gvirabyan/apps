...
class ProductListProvider extends ChangeNotifier {
  ...
  Future<void> bulkDeleteProducts(
    ...
    if (selectedProductIds.isEmpty) {
      setSnackbar('No products selected', context);
      return;
    }
    ...
    if (isNetworkAvail) {
      try {
        final ids = selectedProductIds.join(',');
        var result = await ProductListRepository.bulkDeleteProducts(ids);
  ...
  Future<void> updateProductStatus(
    ...
    if (isNetworkAvail) {
      try {
        var parameter = {"product_id": id, "status": status};
  ...
  deleteProductApi(
    ...
    isNetworkAvail = await isNetworkAvailable();
    if (isNetworkAvail) {
      var parameter = {ProductId: id};
      var result = await ProductListRepository.deleteProductApi(
        parameter: parameter,
      );
