...
class Product {
  ...
  List<String>? tagList = [];
  // Multilingual: language-specific values, { field: { langCode: value } }
  ...
  String? isFav,
      ...
      deliverableZipcodesGroup,
      deliverableCityGroupType,
      deliverableCitiesGroup,
      cancelableTill,
      description,
      downloadAllowed,
      downloadType,
  ...
  Product({
    ...
    this.deliverableGroupType,
    this.deliverableZipcodesGroup,
    this.deliverableCityGroupType,
    this.deliverableCitiesGroup,
    this.cancelableTill,
    this.gurantee,
    this.lowStockLimit,
    this.pickUpLoc,
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    ...
    try {
      if (json['deliverable_cities'].isNotEmpty) {
        deliverableCityList.addAll(
          (json['deliverable_cities'] as List)
    ...
    List<String> otherImage = List<String>.from(
      json["other_images_relative_path"],
    );

    List<String> showOtherimage = List<String>.from(json["other_images"]);

    List<String> selected = [];
    // For non-default languages the API returns translated `tags` as a
    // comma-separated string instead of a list — accept both shapes.
    ...
    // Multilingual: parse { field: { langCode: value } }
    Map<String, Map<String, String>> translations = {};
    if (json['translations'] != null && json['translations'] is Map) {
      (json['translations'] as Map).forEach((field, byLang) {
        if (byLang is Map) {
          translations[field.toString()] = byLang.map(
    ...
    return Product(
      ...
      deliverableGroupType: json['deliverable_group_type'],
      deliverableZipcodesGroup: json['deliverable_zipcodes_group'],
      deliverableCityGroupType: json['deliverable_city_group_type'],
      deliverableCitiesGroup: json['deliverable_cities_group'],
