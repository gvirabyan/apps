...
import 'package:sellermultivendor/Screen/OrderList/Widget/invoicecart.dart';
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            if (widget.order.orderItems.any(
              (element) => (element.itemattachments ?? [])
                  .where((att) => att.trim().isNotEmpty)
                  .isNotEmpty,
            ))
              Column(
                children: [
                  for (var item in widget.order.orderItems)
                    if ((item.itemattachments ?? [])
                        .where((att) => att.trim().isNotEmpty)
                        .isNotEmpty) ...[
                      CardWithTitleDivider(
                        title:
                            '${'ORDER_ATTACHMENTS'.translate(context: context)} - ${item.productName}',
                        child: Column(
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: ListView.separated(
                                ...
                                itemCount: item.itemattachments!
                                    .where((att) => att.trim().isNotEmpty)
                                    .length,
                                ...
                                itemBuilder: (context, index) {
                                  final validAttachments = item.itemattachments!
                                      .where((att) => att.trim().isNotEmpty)
                                      .toList();

                                  final attachmentUrl = validAttachments[index];
            ...
            CardWithTitleDivider(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (!widget.order.isDigitalOrder &&
                        (widget.order.cityName ?? '').isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(bottom: 4),
                        child: Text(
                            '${"City".translate(context: context)}: ${widget.order.cityName}'),
                      ),
                    if (!widget.order.isDigitalOrder &&
                        (widget.order.areaName ?? '').isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(bottom: 8),
                        child: Text(
                            '${"Area".translate(context: context)}: ${widget.order.areaName}'),
                      ),
                    Text(
            ...
            const SizedBox(height: 15),
            InvoiceCard(order: widget.order),
          ],
        ),
      ),
    );
