...
class ConverstationCubit extends Cubit<ConverstationState> {
  ...
  //Silently re-fetch the latest messages and merge any new ones without
  //showing a loader or resetting the list/scroll. Returns true if new
  //messages were added.
  Future<bool> silentRefresh({
    ...
    if (state is! ConverstationFetchSuccess) return false;
    if ((state as ConverstationFetchSuccess).fetchMoreInProgress) return false;
    try {
      final result = await _chatRepository.getConverstation(parameter: {
        'type': isGroup ? 'group' : 'person',
        'to_id': toId,
      ...
      //Silent failure - keep showing existing messages.
      return false;
    }
  }
