...
class ConverstationScreenState extends State<ConverstationScreen>
  ...
  String get _opponentId => widget.isGroup
  ...
  void _startAutoRefresh() {
    ...
    _refreshTimer = Timer.periodic(const Duration(seconds: 2), (_) async {
      if (!mounted) return;
      final hasNew = await context.read<ConverstationCubit>().silentRefresh(
            toId: context.read<UserProvider>().userId!,
            isGroup: widget.isGroup,
            fromUserId: _opponentId,
          );

      //Mark newly arrived personal messages as read.
      if (hasNew && mounted && !widget.isGroup) {
