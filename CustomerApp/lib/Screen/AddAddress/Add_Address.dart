...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        if (widget.update!) {
          ...
          context.read<AddressProvider>().selectedAreaName = item.area;
          context.read<AddressProvider>().areaName = item.area;
          ...
          context.read<AddressProvider>().zipcode = item.pincode;
          ...
          context.read<AddressProvider>().selectedAreaName = null;

          context.read<AddressProvider>().selCityPos = -1;
          context.read<AddressProvider>().selCity = null;
  ...
  bool validateAndSave() {
    ...
    if (form.validate()) {
      ...
      if (context.read<AddressProvider>().selectedAreaName == null ||
          context.read<AddressProvider>().selectedAreaName!.isEmpty) {
        setSnackbar('AREA_REQUIRED_LBL'.translate(context: context), context);
        return false;
      }
            ...
            InkWell(
              onTap: () {
                if (mounted) {
                  ...
                  final selectedItem = provider.zipcodeSearchList[index];
                  stateSetter(
                    () {
                      provider.zipcodeEnable = false;
                      provider.zipcode = selectedItem.zipcode;
                      provider.selectedZipcode = selectedItem.zipcode;
                      // Also populate area name from the area object
                      provider.areaName = selectedItem.name;
                      provider.selectedAreaName = selectedItem.name;
                      provider.area = selectedItem.id;
                    },
                  );
                  Navigator.of(context).pop();
                  setState(() {});
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        provider.zipcodeSearchList[index].zipcode ??
                            provider.zipcodeSearchList[index].name ??
                            '',
                        style:
                      ...
                      if (false) // zipcode field IS the area name, no secondary text needed
                        Text(
                          provider.zipcodeSearchList[index].zipcode!,
                          style: Theme.of(context)
                              .textTheme
                    ...
                    return Column(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.fromLTRB(20.0, 20.0, 0, 0),
                          child: Text(
                            'AREA_NAME_LBL'.translate(context: context),
                            style: Theme.of(this.context)
                                .textTheme
                                .titleMedium!
                                .copyWith(
                ...
                child: GestureDetector(
                  onTap: () {
                    if (context.read<AddressProvider>().selCityPos != null &&
                      ...
                      setSnackbar(
                          'PLZ_START_SEL_CITY_LBL'.translate(context: context),
                          context);
                    }
                  },
                  child: InputDecorator(
                    ...
                    child: Consumer<AddressProvider>(
                      builder: (context, addr, _) => Row(
                        children: [
                          Expanded(
                            child: Text(
                              addr.selectedAreaName != null &&
                                      addr.selectedAreaName!.isNotEmpty
                                  ? addr.selectedAreaName!
                                  : 'AREA_NAME_LBL'.translate(context: context),
                              style: TextStyle(
                                color: addr.selectedAreaName != null
                                    ? Theme.of(context).colorScheme.fontColor
                                    : Colors.grey,
                                fontFamily: 'ubuntu',
                              ),
            ...
            InkWell(
              onTap: () {
                if (mounted) {
                  ...
                  data.selCity = data.citySearchLIst[data.selCityPos!];
      ...
      suffixIcon: IconButton(
        ...
        onPressed: () async {
          ...
          if (onlyOneTimePress) {
            ...
            if (!mounted) return;
            if (context.read<AddressProvider>().latitude == null ||
                context.read<AddressProvider>().latitude!.isEmpty) return;
            ...
            // Auto-fill city from geocoded locality
            final geocodedCity = placemarkItem.locality ?? '';
            final geocodedArea = placemarkItem.subLocality ?? '';
            if (geocodedCity.isNotEmpty && mounted) {
              final provider = context.read<AddressProvider>();
              final cityIndex = provider.citySearchLIst.indexWhere(
                (c) =>
                    (c.name ?? '').toLowerCase() ==
                    geocodedCity.toLowerCase(),
              );
              if (cityIndex != -1) {
                final matchedCity = provider.citySearchLIst[cityIndex];
                provider.selCityPos = cityIndex;
                provider.selectedCity = matchedCity.name;
                provider.city = matchedCity.id;
                provider.selCity = matchedCity;
                provider.cityName = null;
                provider.cityEnable = false;
                // Reset area before loading new ones
                provider.selectedAreaName = null;
                provider.areaName = null;
                provider.area = null;
                provider.selectedZipcode = null;
                provider.zipcode = null;
                if (mounted) setState(() {});

                // Load areas for the matched city
                await provider.getZipcode(
                  matchedCity.id,
                  true,
                  false,
                  context,
                ...
                // Auto-fill area from geocoded subLocality
                if (geocodedArea.isNotEmpty) {
                  final areaIndex = provider.zipcodeSearchList.indexWhere(
                    (a) =>
                        (a.name ?? '')
                            .toLowerCase()
                            .contains(geocodedArea.toLowerCase()) ||
                        geocodedArea
                            .toLowerCase()
                  ...
                  if (areaIndex != -1) {
                    final matchedArea =
                        provider.zipcodeSearchList[areaIndex];
                    provider.areaName = matchedArea.name;
                    provider.selectedAreaName = matchedArea.name;
                    provider.area = matchedArea.id;
                    provider.zipcode = matchedArea.zipcode;
                    provider.selectedZipcode = matchedArea.zipcode;
                    if (mounted) setState(() {});
                  }
                }
              }
  ...
  Widget setPincode() {
    return buildTextFormField(
      ...
      inputType: TextInputType.number,
      inputFormatters: [FilteringTextInputFormatter.digitsOnly],
      onSaved: (val) => context.read<AddressProvider>().zipcode =
