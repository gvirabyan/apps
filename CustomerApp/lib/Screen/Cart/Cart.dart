...
import 'package:eshop_multivendor/Screen/WebView/paytabs_webview.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
            ...
            (getData) async {
              ...
              if (error) {
                ...
                Future.microtask(() {
                  ...
                  // When called from Place Order button (navigate=true), auto-proceed
                  // to confirmation dialog instead of requiring a second tap.
                  if (navigate && mounted) {
                    Future.delayed(const Duration(milliseconds: 100), () {
                      if (mounted && !context.read<CartProvider>().isProgress) {
                        if (context.read<CartProvider>().payMethod == null) {
                          Navigator.push(
                            context,
                            CupertinoPageRoute(
                              builder: (context) => Payment(updateCheckout, ''),
                            ),
                          );
                        } else {
                          confirmDialog();
    ...
    if (isNetworkAvail) {
      ...
      if (context.read<CartProvider>().payMethod ==
        ...
        payVia = 'instamojo';
          ...
          'PAYTABS_LBL'.translate(context: context)) {
        payVia = 'paytabs';
      }

      print('[PLACEORDER] payMethod="${context.read<CartProvider>().payMethod}" → payVia=$payVia');
      print('[PLACEORDER] selAddress=${context.read<CartProvider>().selAddress}');
      print('[PLACEORDER] totalPrice=${context.read<CartProvider>().totalPrice}');

      if (payVia == null) {
        print('[PLACEORDER] ERROR: payVia is null, aborting');
        context.read<CartProvider>().setProgress(false);
        setSnackbar('somethingMSg'.translate(context: context), context);
        return;
      }
      ...
      try {
        ...
        var responseData = await response.stream.toBytes().timeout(const Duration(seconds: timeOut));
        var responseString = String.fromCharCodes(responseData);
        if (response.statusCode != 200) {
          // Extract SQL error from CodeIgniter HTML error page (logcat truncates long strings)
          final errMatch = RegExp(r'<p>(.*?)</p>', dotAll: true).allMatches(responseString);
          final errText = errMatch.map((m) => m.group(1)?.replaceAll(RegExp(r'<[^>]+>'), '').trim()).where((s) => s != null && s!.isNotEmpty).join(' | ');
          print('[PLACEORDER] statusCode=${response.statusCode}');
          print('[PLACEORDER] ERROR DETAILS: $errText');
        } else {
          print('[PLACEORDER] statusCode=200');
        }
        context.read<CartProvider>().placeOrder = true;
        if (response.statusCode == 200) {
          ...
          print('[PLACEORDER] unexpected status ${response.statusCode}');
          context.read<CartProvider>().setProgress(false);
          if (mounted) {
            setSnackbar('somethingMSg'.translate(context: context), context);
          }
        ...
        print('[PLACEORDER] TimeoutException');
        if (mounted) {
          context.read<CartProvider>().checkoutState!(() {
            context.read<CartProvider>().placeOrder = true;
          });
        ...
        print('[PLACEORDER] UNHANDLED EXCEPTION: $e');
        print('[PLACEORDER] STACK: $stack');
        context.read<CartProvider>().setProgress(false);
        if (mounted) {
          context.read<CartProvider>().checkoutState!(() {
            context.read<CartProvider>().placeOrder = true;
  ...
  Future<void> paytabsPayment(String orderId) async {
    try {
      var parameter = {
        ORDER_ID: orderId,
        AMOUNT: context.read<CartProvider>().totalPrice.toString(),
      };
      print('[PAYTABS] calling API orderId=$orderId amount=${context.read<CartProvider>().totalPrice}');
      ...
      final getdata = await apiBaseHelper.postAPICall(paytabsWebviewApi, parameter);
      print('[PAYTABS] response: $getdata');
      bool error = getdata['error'];
      String? msg = getdata['message'];
      if (!error) {
        String? url = getdata['link'];
        print('[PAYTABS] payment url: $url');
        if (url != null && url.isNotEmpty) {
          context.read<CartProvider>().setProgress(false);
          // Keep button disabled while webview is active
          if (mounted) {
            context.read<CartProvider>().checkoutState?.call(() {
              context.read<CartProvider>().placeOrder = false;
            });
          }
          await Navigator.push(
            context,
            CupertinoPageRoute(
              builder: (BuildContext context) => PaytabsWebview(
                url: url,
                from: 'order',
                orderId: orderId,
              ),
          ...
          // Webview was dismissed (failure or back press).
          // On success, cart is cleared and user navigated away — re-enable is a no-op.
        ...
        print('[PAYTABS] error from API: $msg');
        deleteOrder(orderId);
        setSnackbar(msg!, context);
        context.read<CartProvider>().setProgress(false);
        _reEnableButton();
      ...
      print('[PAYTABS] TimeoutException');
      setSnackbar('somethingMSg'.translate(context: context), context);
      context.read<CartProvider>().setProgress(false);
      if (mounted) {
        context.read<CartProvider>().checkoutState?.call(() {
            ...
            child: AlertDialog(
              ...
              actions: <Widget>[
                ...
                TextButton(
                  ...
                  onPressed: () {
                    final payMethod = context.read<CartProvider>().payMethod;
                    final isBankTransfer = payMethod ==
                        'BANKTRAN'.translate(context: context);
                    Routes.pop(context);
